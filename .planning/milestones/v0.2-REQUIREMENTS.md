# Requirements Archive: v0.2 Reliability and Self-Healing
...
- [x] **IPC-01**: Daemon detects that the named HAL shared-memory object has been
  recreated while it is still mapped to an old object. — validated automated
- [x] **IPC-02**: Daemon handles stale shared-memory identity by remapping on a
  non-real-time path or exiting with a distinct recoverable status and stderr
  message. — validated automated (exit 42)
- [x] **IPC-03**: App classifies the daemon's stale-ring recovery signal as
  recoverable and relaunches within the bounded retry policy. — validated
  automated
- [ ] **IPC-04**: Verification proves that the repo build, installed HAL driver,
  installed app helper, and live shm ring build IDs all match. — **accepted
  gap**: automated checks pass; installed proof pending driver reinstall
...
- [ ] **QA-03**: Live verification covers `scripts/verify-hal-driver.sh`,
  ...
  recovery, and a Cubase HAL smoke/soak. — **accepted gap**: checklist in
  `08-LIVE-VERIFICATION.md` pending operator sign-off
...
| IPC-02 | Phase 7 | Complete |
| IPC-03 | Phase 7 | Complete |
| IPC-04 | Phase 7 | Accepted gap |
...
| QA-03 | Phase 8 | Accepted gap |

**Coverage:**
- v0.2 requirements: 24 total
- Satisfied at close: 22 (automated evidence)
- Accepted gaps: 2 (QA-03, IPC-04)
- Mapped to phases: 24
- Unmapped: 0
...
|---------|--------------|
| Validated | APP-01–05, REC-01–05, IPC-01–03, AUD-01–07, QA-01–02 |
| Accepted gap | QA-03 (live operator checklist), IPC-04 (installed build-ID proof) |

---
*Requirements defined: 2026-06-11*
*Archived: 2026-06-11 after v0.2 milestone completion with accepted audit gaps*
