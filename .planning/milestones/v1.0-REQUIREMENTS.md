# Requirements Archive: v1.0 Realtime Race Blocker Closure
...
- [x] **HALIO-01**: `ShmIoHandler::ioRunning_` is a `std::atomic<bool>` shared
  across HAL start, stop, and mixed-output callbacks.
- [x] **HALIO-02**: `OnStartIO`, `OnStopIO`, and `OnProcessMixedOutput` use an
  explicit memory-ordering contract for the stopped-IO guard.
- [x] **HALIO-03**: Regression tests or source-audit guards fail if
  `ioRunning_` returns to a plain `bool`.
- [x] **HALIO-04**: Existing stopped-IO behavior remains intact: mixed-output
  callbacks do not push frames when IO is not running.

### Mono-Lane Callback Serialization

- [x] **MONO-01**: The mono-lane pending queue has a source comment citing the
...
- [x] **MONO-02**: Tests exercise the expected serialized mono-lane callback
  pattern, including compatible lane pairing and old unmatched lane drops.
- [x] **MONO-03**: A source-audit guard fails if shared mono-lane callback state
  exists without an explicit serialization contract or thread-safe redesign.
- [x] **MONO-04**: The existing timestamp/logical-sample mismatch protections
  remain covered after the serialization proof or redesign.

### Verification Closure

- [x] **QA-01**: Source-audit tests cover the named blocker contracts:
...
- [x] **QA-02**: Full repo CI (`scripts/ci.sh`) passes after the realtime race
  blocker fixes.
- [x] **QA-03**: Installed app/helper/driver synchronization checks remain
  green through the repo's trusted verification entrypoints.
- [x] **QA-04**: Release validation records final operator-owned Cubase 15 and
...
|-------------|-------|--------|
| DRIFT-01 | Phase 38 | Complete |
| DRIFT-02 | Phase 38 | Complete |
| DRIFT-03 | Phase 38 | Complete |
| DRIFT-04 | Phase 38 | Complete |
| DRIFT-05 | Phase 38 | Complete |
| HALIO-01 | Phase 39 | Complete |
| HALIO-02 | Phase 39 | Complete |
| HALIO-03 | Phase 39 | Complete |
| HALIO-04 | Phase 39 | Complete |
| MONO-01 | Phase 40 | Complete |
| MONO-02 | Phase 40 | Complete |
| MONO-03 | Phase 40 | Complete |
| MONO-04 | Phase 40 | Complete |
| QA-01 | Phase 41 | Complete |
| QA-02 | Phase 41 | Complete |
| QA-03 | Phase 41 | Complete |
| QA-04 | Phase 41 | Complete |
