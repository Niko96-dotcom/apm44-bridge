...
**Platform:** macOS 14+ (Sonoma and later; validate on macOS 15 Sequoia for HAL signing)  
**Overall stack confidence:** HIGH for Apple HAL/virtual-device path; MEDIUM for distribution/signing edge cases on fresh macOS 15 installs
...
**Driver ↔ daemon handoff:** POSIX shared memory (`shm_open` + `mmap`) + **lock-free SPSC ring** (project-owned in `Shared/`). Apple’s plug-in guidance explicitly recommends mapped memory + lock-free rings for bulk audio between processes ([Cross-Architecture Plug-in Support](https://developer.apple.com/library/archive/documentation/Darwin/Conceptual/64bitPorting/Cross-ArchitecturePluginSupport/Cross-ArchitecturePluginSupport.html)). Driver **must not** open AirPods or run SRC.
...
MVP routing: DAW → **BlackHole 2ch @ 44100** → bridge daemon input IOProc → resample → **AirPods USB-C @ 48000** output IOProc.
...
Daemon runs as **user agent** (LaunchAgent or app-spawned child), not inside `coreaudiod`.
...
**Integrate libsamplerate:** vendor as **git submodule** at tag **0.2.2** or link via Homebrew only for dev; ship **static `.a`** in release daemon for reproducible builds.

**Nominal ratio:** `src_ratio = 48000.0 / 44100.0`; drift nudges ratio around that via controller (smooth transition unless `src_set_ratio` step needed).
...
App **does not** perform HAL I/O; it configures daemon via XPC and reads meter snapshots from shared atomic state updated on a timer thread.
...
```
APM44Bridge.xcworkspace
├── Driver/APM44Bridge.driver/     # libASPL + IO handler → shm ring
├── BridgeDaemon/                  # HAL IOProcs, SRC, drift
├── App/APM44 Bridge/              # SwiftUI MenuBarExtra
└── Shared/                        # RingBuffer, formats, shm layout
```
...
**Entitlements:** No special audio entitlement for user-space HAL plug-in beyond standard Hardened Runtime; **do not** request DriverKit/USB entitlements for this virtual device.

**Installer (v1):** Signed `.pkg` or `.dmg` with `postinstall` script (root) copying `.driver` + kickstart `coreaudiod`; app in `/Applications`.
...
**Universal binary:** Build `arm64` + `x86_64` for `.driver`, daemon, and app (`ARCHS_STANDARD`).

**AirPods Max USB-C:** Treat as **48 kHz–only** endpoint; bridge output IOProc negotiates 48 kHz Float32 2ch.
...
brew install libsamplerate cmake
...
git submodule add https://github.com/libsndfile/libsamplerate.git ThirdParty/libsamplerate
cd ThirdParty/libsamplerate && git checkout 0.2.2
